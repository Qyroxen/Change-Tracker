...
> Change-Tracker - Open source tool by AetherCodeHQ

`changes` `cli` `git` `golang` `tracking`

## What is it?

**Change Tracker** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Change-Tracker.git
cd Change-Tracker
go build -o Change-Tracker .
```
...
```bash
# Basic usage
./Change-Tracker --path ./target

# With options
./Change-Tracker --path ./target --format json --output report.json

# Verbose mode
./Change-Tracker --path ./target --verbose
```
...
### Basic scan
```bash
./Change-Tracker --path ./my-project
```

### JSON report
```bash
./Change-Tracker --path ./my-project --format json --output report.json
```
...
- name: Run Change Tracker
  run: ./Change-Tracker --path . --format json --output report.json
```
